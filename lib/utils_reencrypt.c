	...
	uint64_t device_size)
{
	...
	log_dbg("reencrypt-direction: %s", rh->direction == FORWARD ? "forward" : "backward");
...
int LUKS2_reenc_load(struct crypt_device *cd,
	...
	uint64_t device_size)
{
	return _reenc_load(cd, hdr, rh, device_size);
}
	...
	unsigned commit)
{
	...
	if (r) {
		log_dbg("Failed to assign reencryption moved backup segment.");
		return r;
	}
...
static int _encrypt_set_segments(struct crypt_device *cd, struct luks2_hdr *hdr, uint64_t dev_size, int64_t data_shift)
{
	...
	if (data_shift) {
		...
		first_segment_offset = dev_size - imaxabs(data_shift);
		first_segment_length = imaxabs(data_shift);
		second_segment_offset = imaxabs(data_shift);
		second_segment_length = dev_size - 3 * imaxabs(data_shift);
	} else {
		...
		const struct crypt_params_luks2 *params)
{
	...
	if (digest_new >= 0) {
		jobj_segment_new = LUKS2_segment_create_crypt(
							data_offset * SECTOR_SIZE + imaxabs(data_shift),
...
static int continue_reencryption(struct luks2_reenc_context *rh, uint64_t device_size)
{
	...
	if (rh->type == DECRYPT || (rh->type == ENCRYPT && rh->data_shift))
		...
		*/
	log_dbg("rh->progress: %zu, device_size %zu", rh->progress, device_size);
...
#define DEBUG_ZERO 1
	...
	data_dev_diff = LUKS2_reencrypt_data_dev_diff(hdr);
...
#ifdef DEBUG_ZERO
	zero_buffer = malloc(LUKS2_get_reencrypt_buffer_length(&rh));
	if (!zero_buffer)
		return -ENOMEM;
	memset(zero_buffer, 0, LUKS2_get_reencrypt_buffer_length(&rh));
#endif

	reenc_buffer = aligned_malloc((void **)&reenc_buffer, LUKS2_get_reencrypt_buffer_length(&rh), device_alignment(crypt_data_device(cd)));
	if (!reenc_buffer)
		return -ENOMEM;
	...
	//device_size += data_dev_diff;

	log_dbg("data_dev_diff: %" PRIi64, data_dev_diff);
	...
	while (continue_reencryption(&rh, device_size)) {
		...
		*/
		...
		if (!rh.offset && rh.type == ENCRYPT && rh.data_shift) {
			...
			log_dbg("This will be encryption last step. Changing read_offset to %" PRIu64 " and length to %" PRIu64, read_offset, rh.length);
		} else
			read_offset = rh.offset;
