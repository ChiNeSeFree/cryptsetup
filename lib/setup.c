...
static int _crypt_load_tcrypt(struct crypt_device *cd, struct crypt_params_tcrypt *params)
{
	...
	r = TCRYPT_read_phdr(cd, &cd->u.tcrypt.hdr, &cd->u.tcrypt.params, &cd->u.tcrypt.cipher_spec);
...
int crypt_resize(struct crypt_device *cd, const char *name, uint64_t new_size)
{
	...
	if (new_size == dmd.size) {
		...
		dmd.size = new_size;
		dmd.flags |= CRYPT_ACTIVATE_REFRESH | CRYPT_ACTIVATE_SHARED;
		if (isTCRYPT(cd->type))
			r = -ENOTSUP;
		else if (isLUKS2(cd->type))
			r = LUKS2_unmet_requirements(cd, &cd->u.luks2.hdr, 0, 0);
		if (!r)
			r = _reload_device(cd, name, cd->type, &dmd);
	}
out:
	if (dmd.target == DM_CRYPT)
		crypt_free_volume_key(dmd.u.crypt.vk);
...
const char *crypt_get_cipher_spec(struct crypt_device *cd)
{
	if (!cd)
		...
		return cd->u.tcrypt.cipher_spec;
	...
	uint64_t keyslots_size)
{
	...
	if (metadata_size) {
		r = LUKS2_check_metadata_area_size(metadata_size);
		if (r < 0)
			return r;
	}

	if (keyslots_size) {
		r = LUKS2_check_keyslots_area_size(keyslots_size);
		if (r < 0)
			return r;
	}
	...
	uint64_t *keyslots_size)
{
	...
	if (!cd->type) {
		...
		msize = LUKS2_hdr_and_areas_size(cd->u.luks2.hdr.jobj) / 2;
